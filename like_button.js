...
export function toggle(event, node, key) {
  event.preventDefault(); //const node = ReactDOM.findDOMNode(this);
...
export function getErrorMessage(input) {
  ...
  if (out && typeof out === 'object') {
    ...
    for (let key in out) {
      out2.push(React.createElement('li', {
        key
      }, out[key]));
    }
...
export function getPatientName(patient) {
  ...
  return [prefix.map(t => String(t || '').trim()).join(' '), given.map(t => String(t || '').trim()).join(' '), family.map(t => String(t || '').trim()).join(' '), suffix.map(t => String(t || '').trim()).join(' ')].filter(Boolean).join(' ');
}
/**
...
export function getPatientHomeAddress(patient = {}) {
  ...
  return [a.line, a.postalCode, a.city, a.country].map(x => String(x || '').trim()).filter(Boolean).join(' ');
}
/**
